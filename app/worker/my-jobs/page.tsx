      ...
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <div className="p-5 space-y-0.5">
          {[
            ...
            ...(job.amount ? [{ icon: <CreditCard className="w-3.5 h-3.5" />, label: "Agreed Price", value: `₦${job.amount.toLocaleString()}` }] : []),
          ...
          {job.description && (
            <div className="bg-gray-50 rounded-xl p-3.5 mt-2">
        ...
        <div className="p-5 pt-0 space-y-2">
          ...
          {job.status === "pending" && (
            <button onClick={() => act("start")} disabled={loading === "start"}
          ...
          {job.status === "completed" && (
            <div className="bg-[#f0fdf4] border border-[#bbf7d0] rounded-xl p-4 flex items-center gap-2">
              ...
              <p className="text-xs text-[#065f46] font-medium">Job complete. Payment will be released to your wallet within 24 hours.</p>
            </div>
          )}
...
export default function MyJobsPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, user => {
      ...
      // ─────────────────────────────────────────────────────────────────────
      // FIX: Query jobs where workerId == this worker's uid.
      // This correctly picks up jobs where the client accepted the worker's
      // offer and set workerId on the job document.
      // ─────────────────────────────────────────────────────────────────────
  ...
  const handleAction = async (action: string, job: Job) => {
    try {
      if (action === "start") {
        ...
        toast.success("Job marked complete! Payment will be released soon.");
        setSelected(null);
  ...
  const counts = TABS.reduce<Record<string, number>>((acc, t) => {
    acc[t.id] = t.id === "all" ? jobs.length : jobs.filter(j => j.status === t.id).length;
    return acc;
  }, {});

  const filtered = jobs.filter(j =>
    (activeTab === "all" || j.status === activeTab) &&
    (!search || [j.service, j.category, j.clientName, j.address].some(f => f?.toLowerCase().includes(search.toLowerCase())))
  );
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8">
          <div className="max-w-5xl mx-auto space-y-6">
            ...
            {/* Quick stats — only when there are jobs */}
            {jobs.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  { label: "Pending",     count: pendingCount,                              cls: "bg-yellow-50 border-yellow-200 text-yellow-700" },
                ...
                <AnimatePresence>
                  {filtered.map(job => (
                    <motion.div key={job.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      {/* Colour strip */}
                      <div className={`h-0.5 ${
                        job.status === "pending"     ? "bg-yellow-300" :
                ...
                {search ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-gray-300 mb-5">
                      {activeTab === "all"
                        ? "When a client accepts your offer, the job appears here"
