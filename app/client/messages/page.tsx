...
import {
  Send, Search, Menu, ArrowLeft, X,
  MessageCircle, Lock, CheckCheck, Check,
  Circle, MoreVertical, Loader2
} from "lucide-react";
...
async function getOrCreateChat(clientId: string, workerId: string, clientName: string, workerName: string): Promise<string> {
  ...
  // Fetch worker name from users if not passed
  let wName = workerName;
  if (!wName || wName === "Worker") {
    try {
      const wSnap = await getDoc(doc(db, "users", workerId));
      if (wSnap.exists()) wName = wSnap.data().displayName || wSnap.data().fullName || "Worker";
    } catch {}
  }
  ...
  const chatRef = await addDoc(collection(db, "chats"), {
    ...
    participantDetails: {
      ...
      [workerId]: { name: wName,       role: "worker" },
    },
    paymentConfirmed: true,
    lastMessage:   "Chat started",
    lastMessageAt: serverTimestamp(),
    unreadCount:   { [clientId]: 0, [workerId]: 0 },
...
function MessagesPageInner() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, async snap => {
      ...
      // ?worker=UID — find or create chat
      const workerParam = searchParams.get("worker");
      if (workerParam && currentUser && !selectedChat) {
        ...
        if (found) {
          ...
          setCreating(true);
          try {
            const chatId = await getOrCreateChat(currentUser.uid, workerParam, currentUser.name, "Worker");
            // The onSnapshot will fire again and include the new chat
          } catch {}
          setCreating(false);
        }
      }
    ...
    return () => unsub();
  }, [currentUser?.uid, searchParams.get("worker")]);

  // Auto-select after chats reload with newly created chat
  useEffect(() => {
    const workerParam = searchParams.get("worker");
    if (!workerParam || selectedChat || chats.length === 0) return;
    const found = chats.find(c => c.participants.includes(workerParam));
    if (found) { setSelectedChat(found); setShowChatList(false); }
  }, [chats, searchParams.get("worker")]);
  ...
  const getOtherName = (chat: Chat) => {
    ...
    return chat.participantDetails?.[otherId]?.name || "Worker";
  };
            ...
            <div className="flex-1 overflow-y-auto">
              {creating && (
                <div className="flex items-center gap-3 px-4 py-3 bg-blue-50 border-b border-blue-100">
                  ...
                  <p className="text-xs text-[#0284c7] font-medium">Opening conversation…</p>
                </div>
              )}
              {loading
                ? [...Array(5)].map((_, i) => (
                ...
                : filtered.length > 0
                  ? filtered.map(chat => {
                      ...
                      return (
                        <button key={chat.id}
                          ...
                          className={`w-full flex items-center gap-3 px-4 py-3.5 text-left transition border-b border-gray-50 last:border-0 ${isActive ? "bg-[#e0f2fe]" : "hover:bg-gray-50"}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between">
                              ...
                              {unread > 0 && (
                                <span className="ml-2 w-5 h-5 bg-[#0284c7] text-white text-[10px] font-bold rounded-full flex items-center justify-center shrink-0">{unread}</span>
                              )}
                            </div>
                          </div>
                        </button>
                  ...
                  : (
                    <div className="flex flex-col items-center justify-center h-48 text-center px-4">
                      ...
                      <p className="text-xs text-gray-300 mt-1">Chats appear after a job is booked</p>
                    </div>
                  )}
            </div>
          </div>
          ...
          <div className={`${!showChatList ? "flex" : "hidden"} lg:flex flex-col flex-1 overflow-hidden`}>
            {selectedChat ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-xs text-gray-400">
                        <Circle className="w-2 h-2 text-[#10b981] fill-[#10b981]" /><span>Active</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 bg-gray-50">
                  {messages.length === 0 && (
                    <div className="flex flex-col items-center justify-center h-full text-center">
                      ...
                      <p className="text-xs text-gray-400 max-w-xs">Say hello and discuss the job details with your worker.</p>
                    </div>
                  )}
                  {messages.map((msg, i) => {
                      ...
                      <div key={msg.id}>
                        ...
                        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                          ...
                          <div className={`max-w-[72%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-1`}>
                            <div className={`px-4 py-2.5 rounded-2xl text-sm leading-relaxed ${isMe ? "bg-[#0284c7] text-white rounded-br-sm shadow-sm" : "bg-white text-gray-700 rounded-bl-sm shadow-sm border border-gray-100"}`}>
                              {msg.text}
                            </div>
                            <div className="flex items-center gap-1">
                              {isMe && (msg.read ? <CheckCheck className="w-3 h-3 text-[#0284c7]" /> : <Check className="w-3 h-3 text-gray-400" />)}
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-gray-50">
                ...
                <p className="text-sm text-gray-400 max-w-xs leading-relaxed">Select a conversation from the list or open one from a booking.</p>
              </div>
            )}
          </div>
        </div>
